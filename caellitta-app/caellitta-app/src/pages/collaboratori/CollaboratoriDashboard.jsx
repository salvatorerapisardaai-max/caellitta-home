...
export default function CollaboratoriDashboard() {
  ...
  useEffect(() => {
    ...
    sb.auth.getUser().then(({ data: { user } }) => {
      ...
      sb.from('collaborators').select('name,default_commission_pct').eq('email', user.email).maybeSingle()
        .then(({ data }) => setProfile(data || null))
    })
  }, [])

  async function loadTasks() {
    const { data } = await sb.rpc('get_collaborator_tasks')
    setTasks(data || [])
  }

  async function markTask(bookingId, task) {
    const { error } = await sb.rpc('mark_booking_task', { p_booking_id: bookingId, p_task: task })
  ...
  async function loadMyBookings() {
    const { data } = await sb.rpc('get_my_collaborator_bookings')
    setMyBookings(data || [])
  }
  ...
  const t = {
    it: {
      ...
      newBooking: '+ Blocca date', myBookings: 'Le tue prenotazioni', commission: 'Commissione',
    ...
    en: {
      ...
      newBooking: '+ Block dates', myBookings: 'Your bookings', commission: 'Commission',
