...
export default function Prenotazioni() {
  ...
  // Accoda gli invii ISTAT Sicilia + Alloggiati Web (arrivo/partenza, capofamiglia+ospiti)
  // per una prenotazione confermata/completata. Restano "da approvare" (reviewed=false)
  // finché l'host non li rivede in Adempimenti — nessun invio automatico alla cieca.
  async function enqueueIstatIfNeeded(bookingId, status) {
    if (status !== 'confirmed' && status !== 'completed') return
    const { error: e1 } = await sb.rpc('enqueue_istat_for_booking', { p_booking_id: bookingId })
    if (e1) console.error('enqueue_istat_for_booking:', e1.message)
    const { error: e2 } = await sb.rpc('enqueue_alloggiati_for_booking', { p_booking_id: bookingId })
    if (e2) console.error('enqueue_alloggiati_for_booking:', e2.message)
  }
