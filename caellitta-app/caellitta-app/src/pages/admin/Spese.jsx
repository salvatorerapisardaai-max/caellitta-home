...
const CAT_COLORS = {
  Pulizie: '#3d7a52', Manutenzione: '#a8453f', Utenze: '#2f7a94',
  Biancheria: '#9c7a2e', Materiali: '#9c7a3c', Commissioni: '#7a5a9c', Altro: '#888'
}
...
export default function Spese() {
  ...
  async function load() {
    const { data, error } = await sb.from('expenses').select('*').eq('property_id', activePropertyId).order('date', { ascending: false })
  ...
  async function save() {
    ...
    try {
      if (editing) {
        ...
        const { error } = await sb.from('expenses').insert({ ...payload, property_id: activePropertyId })
        if (error) throw error
      }
      await load()
      setModal(false)
  ...
  return (
    <div>
      ...
      <div className="spese-layout">
        <div className="spese-top">
          ...
          <div className="card spese-cat">
              ...
              return (
                <div key={cat} style={{ marginBottom: '0.8rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', marginBottom: '0.3rem' }}>
                    ...
                    <span style={{ color: 'var(--red)', fontFamily: "'Cormorant Garamond',serif" }}>€{total.toFixed(0)}</span>
                  </div>
            ...
            <div style={{ marginTop: '1.2rem', paddingTop: '1rem', borderTop: '1px solid var(--gold-dim2)', display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '0.65rem', color: 'var(--salt-faint)', textTransform: 'uppercase', letterSpacing: '0.15em' }}>Totale</span>
              <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '1.2rem', color: 'var(--red)' }}>€{totalFiltered.toFixed(2)}</span>
            </div>
          </div>
          ...
          <div className="spese-list">
            {filtered.length === 0 ? (
              ...
              <div key={e.id} style={{
                background: 'var(--lava-card)', border: '1px solid var(--gold-dim2)',
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
                    <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '1rem', color: 'var(--red)' }}>−€{e.amount?.toFixed(2)}</span>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Modifica spesa' : 'Nuova spesa'}>
        {saveError && (
          <div style={{ background: 'rgba(168,69,63,.12)', border: '1px solid rgba(168,69,63,.4)', padding: '0.75rem 1rem', marginBottom: '1rem', fontSize: '0.78rem', color: 'var(--red)', lineHeight: 1.6 }}>
            {saveError}
          </div>
        )}
